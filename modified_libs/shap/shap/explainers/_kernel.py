...
from ..utils._legacy import convert_to_instance_with_index, convert_to_link, IdentityLink, convert_to_data, DenseData, SparseData, DocumentData
...
class Kernel(Explainer):
    ...
    def __init__(self, model, data, link=IdentityLink(), **kwargs):
        ...
        if isinstance(self.data, DocumentData):
            self.data.special_token = kwargs.get('special_token', None)
        ...
        assert isinstance(self.data, DenseData) or isinstance(self.data, SparseData) or isinstance(self.data, DocumentData), \
        ...
        # init our parameters
        self.N = 1
        # print('Number of background examples: %d' % (self.N))
        if not isinstance(self.data, DocumentData):
        ...
        print('This is the fnull:', self.fnull)
        #a small hack for my use case
        self.data.weights = np.ones(self.data.data.shape[0])
        # print('This is the fnull shape:', self.fnull.shape)
    ...
    def shap_values(self, X, **kwargs):
        ...
        if str(type(X)).endswith("pandas.core.series.Series'>"):
            ...
            X = X.values
        elif all([type(x) == str or type(x) == np.str_ for x in X]):
            pass
        ...
        # single instance
        if len(X.shape) == 1 and not isinstance(self.data, DocumentData):
        ...
        elif len(X.shape) == 2 or isinstance(self.data, DocumentData):
            explanations = []
    ...
    def transform_to_vector(self, x):
        return np.array(self.data.tokenizer.encode(x[0]), dtype=np.int).reshape(1, -1)
    
    def transform_to_document(self, token_data):
        # print('before filtering:', token_data[:5, :100])
        transformed_data = [self.data.tokenizer.decode(tokens[:np.where(tokens==-1)[0][0]]) for tokens in token_data]
        return np.array(transformed_data)

    def explain(self, incoming_instance, **kwargs):
        # convert incoming input to a standardized iml object
        # print('The shape of input to explain:', incoming_instance.shape)
        ...
        # print('shape of varying indices:', self.varyingInds.shape)
        # print('varying indices found:', self.varyingInds)
        
        instance_data = instance.x
        if isinstance(self.data, DocumentData):
            instance_data = self.transform_to_vector(instance.x)
            self.instance_length = instance_data.shape[-1]
        # print('This is the instance length:', self.instance_length)
        # print('This is the instance data')
        ...
        # print('This is the model output:', model_out.shape)
        ...
        # if more than one feature varies then we have to do real work
            ...
            for subset_size in range(1, num_subset_sizes + 1):
                ...
                if num_samples_left * remaining_weight_vector[subset_size - 1] / nsubsets >= 1.0 - 1e-8:
                    ...
                    # print('this is the weoght vector:', w, w.shape)
                    if subset_size <= num_paired_subset_sizes: w /= 2.0
                    for inds in itertools.combinations(group_inds, subset_size):
                        # print(inds)
                        mask[:] = 0.0
                        mask[np.array(inds, dtype='int64')] = 1.0
                        self.addsample(instance_data, mask, w)
            ...
            if num_full_subsets != num_subset_sizes:
                ...
                while samples_left > 0 and ind_set_pos < len(ind_set):
                    ...
                    if mask_tuple not in used_masks:
                        ...
                        samples_left -= 1
                        self.addsample(instance_data, mask, 1.0)
                    else:
    ...
    def varying_groups(self, x):
        if all([type(conv)==str or type(conv)==np.str_ for conv in x]):
            # print(x[0][0])
            # print(self.data.tokenizer)
            # print('The input to find varying indices:', x)
            # print(self.data.tokenizer.encode(x[0]))
            varying_mask = np.array(self.data.tokenizer.encode(x[0])) != self.data.special_token
            return np.array([i for i in range(len(self.data.tokenizer.tokenize(x[0])))])[varying_mask]
        elif not sp.sparse.issparse(x):
    ...
    def allocate(self, instance_data=None):
        ...
        elif isinstance(self.data, DocumentData):
            # self.synth_data = \
            #     np.stack([self.data.sample_from_tokens() for i in range(self.nsamples)], axis=0).\
            #         reshape(self.nsamples * self.data.data.shape[0], -1)
            # print('Number of samples:', self.nsamples)
            self.synth_data = np.tile(self.data.sample_from_tokens(instance_data), (self.nsamples, 1))
            # print('This is the synthetic data:', self.synth_data[:5,])
            # print('Synthetic Data Shape:', self.synth_data.shape)
        else:
    ...
    def addsample(self, x, m, w):
        # print('In add sample function:', x, m , w)
        ...
        if isinstance(self.varyingFeatureGroups, (list,)):
            ...
            if len(groups.shape) == 2:
                ...
                # print('Groups to index:', groups)
                # print('Offset:', offset, 'Number of items in synthetic data:', self.N)
                ...
                if isinstance(self.data, DocumentData):
                    start_ = 0
                    for i in list(groups) + [x.shape[1]]:
                        empty_indices = np.arange(start_, i)
                        # print('Empty Indices:', empty_indices)
                        self.synth_data[offset:offset+self.N, empty_indices] = 50256
                        spcl_tkn_pos_start = 1 if start_ == 0 else start_
                        spcl_tkn_pos = np.arange(spcl_tkn_pos_start, i, 2)
                        # print('Special Token Indices:', spcl_tkn_pos)
                        self.synth_data[offset:offset+self.N, spcl_tkn_pos] = self.data.special_token
                        start_ = i + 1
    ...
    def run(self):
        ...
        # print('This is the final synthetic dataset:', self.synth_data)
        if isinstance(self.data, DocumentData):
            data = self.transform_to_document(data)
        if self.keep_index:
