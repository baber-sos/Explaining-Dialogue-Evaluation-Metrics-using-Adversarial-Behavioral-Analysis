...
def match_model_to_data(model, data, **kwargs):
    ...
    try:
        ...
        elif isinstance(data, DenseDataWithIndex):
...
class DocumentData(Data):
    def __init__(self, data, tokenizer, *args):
        ...
        self.sampling_tokens = dict()
        # self.tokens_info = np.ones(data.shape[0], self.max_tokens) * -1
        self.special_token = None
        self.tokenizer = tokenizer
        # self.background_data = np.ones((data.shape[0], self.max_tokens), dtype=np.int) * -1
        self.total_tokens = 0
        self.bgd_lens = []
        for i, document in enumerate(data):
            tokens = tokenizer.encode(document[0])
            self.bgd_lens.append(len(tokens))

            for token in tokens:
                # self.tokens_info[i, :len(tokens)] = tokens
                if token == 220:
                    continue
                self.sampling_tokens.setdefault(token, 0)
                self.sampling_tokens[token] += 1
                self.total_tokens += 1
        self.data = data
        print('Data Shape:', self.data.shape)
        self.weights = np.ones(len(data))/len(data)
        self.transposed = False
        self.groups = [np.array([i]) for i in range(self.max_tokens)]
        self.groups_size = self.max_tokens #this encodes the max sequence length
    
        # self.background_data = self.sampling_from_tokens()

    def sample_from_tokens(self, instance_data):
        random_data = np.ones((instance_data.shape[0], self.max_tokens), dtype=np.int) * -1
        # random_data[:, :instance_data.shape[1]] = 50256
        # special_tokens = np.arange(1, instance_data.shape[1], 2)
        # random_data[:, special_tokens] = 220
        return random_data
        # tokens_dictionary, tokens_weights = (list(self.sampling_tokens.keys()), \
        #     np.array([self.sampling_tokens[k]/self.total_tokens for k in self.sampling_tokens.keys()]))
        # for i in range(self.data.shape[0]):
        #     # sent_length = random.randint(1, instance_length-1)
        #     random_data[i, :instance_length] = np.random.choice(tokens_dictionary, instance_length, p=tokens_weights)
        # # print('background data shape:', random_data.shape)
        # return random_data
...
def convert_to_data(val, keep_index=False):
    if isinstance(val, Data):
        ...
        #These would be the background text documents. A list of strings.
    ...
    # elif type(val) == list and all([type(x)==str or type(x)==np.str_  for x in val]):
