...
def format_conversation(conversation, utterance_starters=['User:', 'System:']):
  utterances = conversation.split('\n')
  separator_token = '<|endoftext|>'
  for i in range(len(utterances)):
    if any([utterances[i].startswith(starter) for starter in utterance_starters]):
      utterances[i] = utterances[i].split(':')[1].strip()
    
  return f' {separator_token} '.join(utterances)
...
def fix_length_issues(tokenized_input):
  #50256, 220
  end_sent_tokens = []
  ...
  for i, token in enumerate(tokenized_input):
    if token == 50256:
      end_sent_tokens.append(i)
  
  tokens_to_remove = len(tokenized_input) - 1024
  for token_loc in end_sent_tokens:
    if token_loc >= 2 and token_loc < (len(tokenized_input) - 1) and tokenized_input[token_loc - 1] == 220 and \
      tokenized_input[token_loc - 2] == 50256 and tokenized_input[token_loc + 1] == 220:
      tokenized_input[token_loc] = -1
      tokenized_input[token_loc-1] = -1
      tokens_to_remove -= 2

    if tokens_to_remove <= 0:
      break
  
  return list(filter(lambda x: x != -1, tokenized_input))
...
def score(text, tokenizer, model):
  ...
  tokenized_input = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(text))
  # print('number of tokens before:', len(tokenized_input))
  tokenized_input = fix_length_issues(tokenized_input)
  # print('number of tokens after:', len(tokenized_input))
  #50256 is the token_id for <|endoftext|>
  tensor_input = torch.tensor([tokenized_input]).cuda()
  with torch.no_grad():
